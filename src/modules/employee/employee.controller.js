...
import { addDailyLog, attendanceList, checkIn, checkOut, dailyLogsList, latestLog, updateAttendance } from "./employee.service.js";
...
export const dailyLogsController = async (req, res) => {
    ...
    const { data, total } = await dailyLogsList(userId);
    sendSuccess(res, { data, meta: { page, limit, total } });
}
...
// get latest logs controller
export const latestLogController = async (req, res) => {
    const { log } = await latestLog(req.params.userId);
    sendSuccess(res, { data:log });
}
